...
总览页的“检查更新”只读取 GitHub `main` 的提交信息；发现新版本后点击“下载更新”，更新包会先保存到状态目录，不会替换正在运行的代码，也不会重启服务。归档任务空闲后点击“安装并重启”，Ubuntu 更新助手会再次检查没有下载或校验任务，再替换运行文件并重启 `smsi-archive-client.service`。更新助手只接受固定版本号和固定运行目录，不能执行任意命令。首次安装或更新助手变更时，仍需重新运行 `deploy/install_ubuntu.sh`。
