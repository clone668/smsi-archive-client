...
告警不在数据路径上：发送失败会在下一轮检查时自动重试，不会让同步、下载或校验失败。同一个失败原因只在事件日志记录一次，最近一次失败原因显示在“设置”页面的告警状态行，因为通道本身坏掉时无法用它通知自己。消息使用纯文本并在超长时截断，避免格式解析导致整条消息被拒收。机器人令牌只保存在状态目录，接口只回显末四位，保存其他设置时留空即保持不变。“发送测试消息”只验证通道当前是否可用，不推进告警游标，也不影响待推送内容。
